...
public class MqlParser extends Parser {
    ...
    protected final String MQL_TMP_IN_VALUE_DELIMITER = "_____";
    protected final String MQL_KEY_VALUE_DELIMITER_SPACE = "\\s+";
    protected final String MQL_KEY_VALUE_DELIMITER_EQ = "\\s*=\\s*";
    protected String mqlKeyValueDelimiter = MQL_KEY_VALUE_DELIMITER_SPACE;
    public final String MQL_NOT_BOOLEAN_PREFIX = "not";
...
//        objectName = (String) keyPathAndValue.get(VALUE);
    ...
    private List<String> removeStartWhitespacesColumn(List<String> lines) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            //todo проверить необходимость переделать цикл ниже через цикл по codePoints
...
//        String[] lines = parsebleBlock.split("\n");
...
//        return currLine.startsWith(beforeKeySpace + currKeyword + mqlKeyValueDelimiter);
...
//        return !(line.isEmpty() || line.matches(mqlKeyValueDelimiter + ".*"));
    ...
    private void addKeyValueToResult(Map<String, Object> values, List<String> keyPath, String foundKeyValue, Function valueMaker) throws ParserException {
    ...
    private void addKeyValueToResult(Map<String, Object> values, String keyFound, String foundKeyValue, Function valueMaker) throws ParserException {
        Object resultValue = (valueMaker == null) ? foundKeyValue : valueMaker.apply(foundKeyValue);

//        if (resultValue instanceof String) { //todo ???
//            int index = 0;
//            String[] strArr = ((String) resultValue).split("");
//
...
//                }
//            }
//            resultValue = ((String) resultValue).substring(0, index);
//        }
        ...
        if (values.containsKey(keyFound)) {
            ...
            if (prevValue instanceof Collection) {
                if (!(resultValue instanceof Collection)) {
                    throw new ParserException(String.format("Incorrect new value %s for key %s. Previous value is collection %s", resultValue, keyFound, prevValue));
                }
    ...
    public static List<String> splitHeaderLine(String line) {
        ...
        /* This code produces bug: line like "'eService Trigger Program Parameters' 'sdvds sdcs' 1" is parsed into -> {"eService Trigger Program Parameters", "sdvds", "sdcs", "1"}
        String value = null;
        final char space = ' ';
        char endPartSymbol = space;
        for (int i = 0; i < line.length(); i++) {
            char currChar = line.charAt(i);
            if (value == null) {
                if (currChar == '\'') {
                    endPartSymbol = currChar;
                    value = "";
                    continue;
                }
                ...
                value = String.valueOf(currChar);
                continue;
            }

            if (currChar == endPartSymbol) {
                parts.add(value);
                value = null;
                continue;
            }
