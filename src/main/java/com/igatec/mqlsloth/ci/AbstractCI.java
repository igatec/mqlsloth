...
import com.igatec.mqlsloth.script.ScriptBuildingException;
...
public abstract class AbstractCI implements MqlKeywords, YAMLKeywords, ScriptPriority {
    ...
    protected static final String MODE_VALIDATION_ERROR = "Mode validation error";
    ...
    public boolean equals(Object o) {
        ...
        if (getDiffMode() != ((AbstractCI) o).getDiffMode()) {
            return false;
        }
        try {
    ...
    public List<ScriptChunk> buildUpdateScript() {
        ...
        try {
            for (Method method : getClass().getMethods()) {
                processBooleanAnnotated(fName, chunks, method);
                processStandaloneStringAnnotated(fName, chunks, method);
                processStringAnnotated(fName, chunks, method);
                processReversibleStringAnnotated(fName, chunks, method);
                processStringSetAnnotated(fName, chunks, method);
            }
    ...
    private void processStringSetAnnotated(CIFullName fName, List<ScriptChunk> chunks, Method method)
    ...
    private void processReversibleStringAnnotated(CIFullName fName, List<ScriptChunk> chunks, Method method)
            throws IllegalAccessException, InvocationTargetException {
    ...
    private void processStringAnnotated(CIFullName fName, List<ScriptChunk> chunks, Method method)
            throws IllegalAccessException, InvocationTargetException {
    ...
    private void processStandaloneStringAnnotated(CIFullName fName, List<ScriptChunk> chunks, Method method)
            throws IllegalAccessException, InvocationTargetException {
    ...
    private void processBooleanAnnotated(CIFullName fName, List<ScriptChunk> chunks, Method method)
            throws IllegalAccessException, InvocationTargetException {
