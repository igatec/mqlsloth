...
import com.igatec.mqlsloth.ci.PolicyAllstateRules;
import com.igatec.mqlsloth.ci.PolicyCI;
import com.igatec.mqlsloth.ci.PolicySignature;
...
import com.igatec.mqlsloth.ci.TriggerKey;
import com.igatec.mqlsloth.ci.TriggerValue;
...
public class PolicyObjectParser extends AdminObjectObjectParser {
    private static final int STATE_MQL_RESULT_INDEX = 4;
    ...
    public Map<String, Function> getKeyWordsMQL() {
        ...
        keyWordsToValueMakers.put(M_MINOR_SEQUENCE_WITH_SPACE, Function.identity());
    ...
    protected void setParsedMQLValuesToObject(Map<String, Object> parsedValues, AbstractCI parsebleObject) {
        ...
        Optional.ofNullable(parsedValues.get(M_MINOR_SEQUENCE_WITH_SPACE)).ifPresent(v -> obj.setMinorSequence((String) v));
    ...
    private void makeStateFromMQL(String ciStateMql) {
        ...
        List<String> stateData = stateDt.stream().map(s -> s.substring(STATE_MQL_RESULT_INDEX)).collect(Collectors.toList());
        ...
        while (stateLinesIter.hasNext()) {
            ...
            } else if (stateRecord.startsWith(M_CHECKOUT_HISTORY_WITH_SPACE) && state instanceof PolicyState) {
    ...
    private void processAccessRecordMQL(PolicyAllstateRules state, String record) {
        ...
        for (first = firstWord(record); !set1.contains(first); first = firstWord(record)) {
            modifs.add(first);
        ...
        for (
                first = firstWord(record, true);
                !record.isEmpty() && !AccessValue.stringValues().contains(first);
                first = firstWord(record, true)
        ) {
        ...
        for (int i = 0; i < userAndKeyWords.size(); i++) {
            if (i < kIndex) {
                userWords.add(userAndKeyWords.get(i));
            }
            if (i > kIndex) {
                keyWords.add(userAndKeyWords.get(i));
            }
        }
        ...
        for (first = firstWord(record); !record.isEmpty() && !(first).equals(M_FILTER); first = firstWord(record)) {
            extraInfo.add(first);
            record = rest(record, first);
        }
        String filter = null;
    ...
    private String firstWord(String record, boolean comma) {
        ...
        if (iSpace == -1) {
            iSpace = Integer.MAX_VALUE;
        }
        int iComma = record.indexOf(",");
        if (iComma == -1) {
            iComma = Integer.MAX_VALUE;
        }
    ...
    private void modifyStateYAML(PolicyAllstateRules state, Map<String, Object> stateMap) {
        ...
        prepareAcess(state, stateMap);
        prepareTriggers(state, stateMap);
        prepareSignatures(state, stateMap);
    }

    private void prepareSignatures(PolicyAllstateRules state, Map<String, Object> stateMap) {
        if (state instanceof PolicyState) {
            PolicyState st = (PolicyState) state;
    ...
    private void prepareAcess(PolicyAllstateRules state, Map<String, Object> stateMap) {
        Map<String, List<String>> accs = (Map<String, List<String>>) Optional.ofNullable(stateMap.get(Y_ACCESS))
                .orElse(new HashMap<>());
        for (String key : accs.keySet()) {
            ...
            for (String val : vals) {
                ...
                if (val.startsWith(M_KEY)) {
                    ...
                    String first;
                    for (
                            first = firstWord(val, true);
                            !val.isEmpty() && !AccessValue.stringValues().contains(first);
                            first = firstWord(val, true)
                    ) {
                        keyWords.add(first);
                        val = rest(val, first);
                ...
                for (
                        first = firstWord(val);
                        !val.isEmpty() && !(first).equals(M_FILTER);
                        first = firstWord(val)
                ) {
                    extraInfo.add(first);
                    val = rest(val, first);
    ...
    private void prepareTriggers(PolicyAllstateRules state, Map<String, Object> stateMap) {
