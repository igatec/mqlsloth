...
import com.igatec.mqlsloth.util.ComposedObjectStreamReader;
import com.igatec.mqlsloth.util.EndOfCollectionStub;
import com.igatec.mqlsloth.util.ObjectContainer;
import com.igatec.mqlsloth.util.ObjectStreamReader;
import com.igatec.mqlsloth.util.ObjectStreamReaderImpl;
    ...
    private class MultithreadSession implements Session {
        ...
        public void run() {
            ...
            service2.submit(() -> {
                try {
                    for (
                            ObjectContainer<CIFullName> nameContainer = names.take();
                            !nameContainer.isStub(); nameContainer = names.take()
                    ) {
                        try {
            ...
            service3.submit(() -> {
                try {
                    for (
                            ObjectContainer<String> ciStringContainer = ciStrings.take();
                            !ciStringContainer.isStub();
                            ciStringContainer = ciStrings.take()
                    ) {
                        try {
